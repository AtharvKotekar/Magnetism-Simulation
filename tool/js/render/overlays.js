...
export class Overlays {
  ...
  buildSolenoidFieldLines(poleA, poleB, rMax, opts = {}) {
    ...
    // Field lines = TRUE STADIUM loops: a straight parallel interior through the
    // bore, straight outer return sides, joined by EXACT SEMICIRCLE caps (the
    // rounded ends of a bar magnet's field, with a solenoid's flat interior).
    // The loop spacing follows a dipole falloff h(n): the outer side of each
    // loop sits at n_out solved from h(n_out) = h(n0). See the seed loop below.
    const Lh = L / 2;
    const na = 3.4 * boreR, al = 1.6;
    ...
    // TRUE STADIUM field lines: straight parallel interior + straight outer
    // sides joined by EXACT SEMICIRCLE caps (round, like a bar magnet's loops),
    // instead of the streamline's square-shouldered flat-top caps. n0 = seed
    // bore radius; n_out solved from h(n_out) = h(n0) so the loop spacing keeps
    // the same dipole falloff as before; cap radius R = (n_out - n0)/2 centred
    // on the pole plane, so each cap is a clean half-circle rising R past the
    // pole. Interior line and exterior loop share their endpoints (connected).
    const nPeak = na * Math.pow(1 / al, 1 / al);
    const nOutFor = (n0) => {
      const target = hFn(n0);
      let lo = nPeak, hi = 40 * boreR;         // decreasing branch: n_out > n_peak
      for (let k = 0; k < 80; k++) {
        const mid = (lo + hi) / 2;
        if (hFn(mid) > target) lo = mid; else hi = mid;
      }
      return (lo + hi) / 2;
    };
    const CAPSEG = 48;
    ...
    for (const seed of [0.14, 0.30, 0.46, 0.62, 0.78]) {
      const n0 = seed * boreR;
      const nO = nOutFor(n0);
      const R = (nO - n0) / 2, cx = (n0 + nO) / 2;
      for (const side of [1, -1]) {
        // interior straight bore line at side*n0, wound BOTTOM -> TOP: that is
        // the field direction inside (N is at the top, u = -Lh).
        const interior = [];
        for (let t = 0; t <= 40; t++) interior.push(emit(Lh - (2 * Lh) * t / 40, side * n0));
        for (const r of clipRuns(interior)) boreLines.push(r);
        // exterior wound in the FIELD direction: exit the TOP (u=-Lh), semicircle
        // cap (inner -> outer), straight outer side DOWN, semicircle cap (outer
        // -> inner) at the BOTTOM. Endpoints coincide with the interior line.
        const ext = [];
        for (let t = 0; t <= CAPSEG; t++) {                 // top cap: pi -> 0
          const th = Math.PI * (1 - t / CAPSEG);
          ext.push(emit(-Lh - R * Math.sin(th), side * (cx + R * Math.cos(th))));
        }
        for (let t = 1; t <= CAPSEG; t++) {                 // outer side, top -> bottom
          ext.push(emit(-Lh + (2 * Lh) * t / CAPSEG, side * nO));
        }
        for (let t = 1; t <= CAPSEG; t++) {                 // bottom cap: 0 -> pi
